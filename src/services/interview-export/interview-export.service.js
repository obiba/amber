...
const makeTableName = (interviewDesign, step, formRevision) => {
  const tableName = `${interviewDesign.name}-${step.name}-${formRevision.revision}`;
  return tableName;
};
...
const doCsvResponse = (res) => {
  ...
  for (const key in res.data.export) {
    const tableName = makeTableName(res.data.export[key].interviewDesign, res.data.export[key].step, res.data.export[key].formRevision);
...
const doJsonResponse = (res) => {
  res.attachment('interview-export.json');
  const data = {};
  for (const key in res.data.export) {
    const tableName = makeTableName(res.data.export[key].interviewDesign, res.data.export[key].step, res.data.export[key].formRevision);
    data[tableName] = {
